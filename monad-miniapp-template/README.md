...
### Environment variables (required before run)

**Never commit `.env`** — it is in `.gitignore`. Use `.env.example` as a template:

```bash
cp .env.example .env
# Edit .env and set at least NEXT_PUBLIC_URL (and any API keys you need).
```

For local dev with Farcaster embed, set `NEXT_PUBLIC_URL` to your public URL (e.g. from ngrok or cloudflared). See [Expose localhost](#view-the-app-in-warpcast-embed-tool) below.
...
#### Set `NEXT_PUBLIC_URL` in your `.env` file
