...
export function NotificationActions() {
  ...
  const [notificationDetails, setNotificationDetails] = useState<MiniAppNotificationDetails | null>(
    null
  )
  ...
  const { mutate: sendNotification, isPending: isSendingNotification } = useMutation({
    ...
    onSuccess: (response) => {
      ...
      else if (response.status === 429) setResult('Rate limited. Try again later.')
        ...
        {notificationDetails ? (
          ...
          <>
            ...
            <p className="text-xs text-red-600">
              You must add this Mini App and enable notifications to send a test notification.
            </p>
          </>
        )}
